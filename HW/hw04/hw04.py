...
def balanced(m):
    ...
    if is_planet(m):
        ...
        return all([balanced(end(left(m))), balanced(end(right(m))), length(left(m)) * total_mass(end(left(m))) == length(right(m)) * total_mass(end(right(m)))])
...
def berry_finder(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    return any([berry_finder(i) if type(i) == list else i == 'berry' for i in t])
...
def max_path_sum(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    return max([t[0] + max_path_sum(t[i]) for i in range(1, len(t))]) if len(t) > 1 else t[0]
